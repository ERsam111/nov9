...
export const MLClusteringNode = memo(({ data, selected }: NodeProps) => {
  return (
    <div className={`px-3 py-2 shadow-md rounded-md bg-card border-2 ${selected ? 'border-primary' : 'border-border'} min-w-[140px] hover:shadow-lg transition-all group`}>
      <div className="flex items-center gap-2">
        <div className="p-1 rounded bg-pink-500/10 group-hover:bg-pink-500/20 transition-colors">
          <Sparkles className="h-3 w-3 text-pink-500" />
        </div>
        <div className="font-semibold text-xs">Clustering</div>
      </div>
...
MLClusteringNode.displayName = 'MLClusteringNode';
