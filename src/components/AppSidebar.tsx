                    ...
                    <div className="px-2 pb-2 border-b">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent className="w-48">
                          <DropdownMenuItem onClick={() => setTypeFilter("all")}>All Projects</DropdownMenuItem>
                          <DropdownMenuItem onClick={() => setTypeFilter("gfa")}>
                            <span
                              className={`px-1.5 py-0.5 rounded text-xs font-semibold mr-2 ${getModuleTagColor("gfa")}`}
                            >
                              GFA
                            </span>
                            GFA Projects
                          ...
                          <DropdownMenuItem onClick={() => setTypeFilter("inventory")}>
                            <span
                              className={`px-1.5 py-0.5 rounded text-xs font-semibold mr-2 ${getModuleTagColor("inventory")}`}
                            >
                              IO
                            </span>
                          ...
                          <DropdownMenuItem onClick={() => setTypeFilter("forecasting")}>
                            <span
                              className={`px-1.5 py-0.5 rounded text-xs font-semibold mr-2 ${getModuleTagColor("forecasting")}`}
                            >
                              DP
                            </span>
                            Demand Planning
                      ...
                      <div className="space-y-1">
                        {filteredProjects.map((project) => {
                          ...
                          return (
                            <div
                              key={project.id}
                              className={`px-2 py-2 text-sm rounded-md cursor-pointer group flex items-center justify-between transition-all border ${isCurrentProject ? `${getModuleHighlightColor(project.tool_type)} border-l-4 font-medium` : "hover:bg-accent border-transparent"}`}
                              onClick={() => handleProjectClick(project.id, project.tool_type)}
                            >
                              <div className="flex items-center gap-2 flex-1 min-w-0">
                                <span
                                  className={`px-1.5 py-0.5 rounded text-xs font-semibold flex-shrink-0 ${getModuleTagColor(project.tool_type)}`}
                                >
                                  {tag}
                                </span>
                                <span className="font-medium truncate">{project.name}</span>
                                {scenarioCount > 0 && (
                                  <span className="text-xs text-muted-foreground flex-shrink-0">({scenarioCount})</span>
                                )}
                              </div>
                              <DropdownMenu>
                                <DropdownMenuTrigger
                                  className="opacity-0 group-hover:opacity-100 ml-2 p-1 hover:bg-accent-foreground/10 rounded flex-shrink-0"
